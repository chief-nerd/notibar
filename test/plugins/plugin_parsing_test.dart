...
void main() {
  group('Plugin Parsing Tests', () {
    ...
    test('SlackPlugin parses conversations correctly', () {
      final fixture = File('test/fixtures/slack_conversations.json').readAsStringSync();
    ...
    test('GithubPlugin parses notifications correctly', () {
      final fixture = File('test/fixtures/github_notifications.json').readAsStringSync();
      final json = {'value': jsonDecode(fixture)};
      ...
      expect(summary.assignedIssuesCount, 1);
      
      expect(summary.items.first.metadata['reason'], 'mention');
      expect(summary.items.first.metadata['type'], 'PullRequest');
    });
    ...
    test('FrappePlugin parses todos correctly', () {
      final fixture = File('test/fixtures/frappe_todos.json').readAsStringSync();
      final json = jsonDecode(fixture);
    ...
    test('MattermostPlugin parses team unreads correctly', () {
      final fixture = File('test/fixtures/mattermost_unreads.json').readAsStringSync();
      final json = {'teams': jsonDecode(fixture)};
      final plugin = MattermostPlugin();
      
      final summary = plugin.parseSummary(json, baseUrl: 'https://mattermost.example.com');
    ...
    test('OutlookPlugin parses messages correctly', () {
      final fixture = File('test/fixtures/outlook_messages.json').readAsStringSync();
      final json = jsonDecode(fixture);
